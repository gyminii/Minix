...
import { ArrowLeft, Check, Clock, Copy, Edit, Folder, Loader2, Save, Share2, Trash2, Code2 } from "lucide-react"
...
export default function PasteViewPage() {
  ...
  useEffect(() => {
    const fetchPaste = async () => {
      try {
        ...
        toast.error(error instanceof Error ? error.message : "Failed to fetch paste")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (expiration !== "never") {
        const now = new Date()
        if (expiration === "30m") expiresAt = new Date(now.getTime() + 30 * 60000)
        else if (expiration === "1h") expiresAt = new Date(now.getTime() + 60 * 60000)
        else if (expiration === "1d") expiresAt = new Date(now.getTime() + 24 * 60 * 60000)
        else if (expiration === "1w") expiresAt = new Date(now.getTime() + 7 * 24 * 60 * 60000)
        else if (expiration === "1m") expiresAt = new Date(now.getTime() + 30 * 24 * 60 * 60000)
      }
      ...
      <div className="container mx-auto max-w-4xl">
        <Card>
          <CardContent className="p-6 text-center">
            <p className="text-muted-foreground">Paste not found or has expired</p>
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible">
      <motion.div variants={itemVariants} className="mb-6 flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div>
            ...
            <div className="flex items-center text-sm text-muted-foreground mt-1">
              <span className="mr-4">Created: {format(new Date(paste.created_at), "MMM d, yyyy h:mm a")}</span>
              {paste.expires_at && (
                <span className="flex items-center">
                  <Clock className="mr-1 h-4 w-4" />
                  Expires: {format(new Date(paste.expires_at), "MMM d, yyyy h:mm a")}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="icon" onClick={handleCopyContent} title="Copy content">
          ...
          <Dialog open={showShareDialog} onOpenChange={setShowShareDialog}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Anyone with this link can view this paste.</DialogDescription>
              </DialogHeader>
              <div className="flex items-center space-x-2 mt-4">
                <Input readOnly value={`${window.location.origin}/paste/${id}`} className="flex-1" />
          ...
          <Button variant="outline" size="icon" onClick={() => setIsEditing(!isEditing)} title="Edit paste">
            <Edit className="h-4 w-4" />
          </Button>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="outline" size="icon" className="text-destructive" title="Delete paste">
        ...
        {isEditing ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Make changes to your paste content, syntax highlighting, or expiration time.
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center">
                ...
                {syntaxOptions.find((o) => o.value === paste.syntax)?.label || "Plain Text"}
                {paste.folder_id && (
                  <div className="ml-4 flex items-center text-sm text-muted-foreground">
                    <Folder className="mr-1 h-4 w-4" />
                    <span>{folders?.find((f) => f.id === paste.folder_id)?.name || "Loading folder..."}</span>
                  </div>
                )}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="rounded-md border overflow-hidden w-full">
                <div className="max-w-full">
                  <SyntaxHighlighter
                    language={paste.syntax === "plaintext" ? "text" : paste.syntax}
                    style={vscDarkPlus}
                    showLineNumbers
                    wrapLines={true}
                    customStyle={{
